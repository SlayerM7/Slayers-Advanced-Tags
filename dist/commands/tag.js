...
const editLastModified_1 = require("../functions/editLastModified");
const tag_operations_1 = require("../utils/tag-operations");
module.exports = {
    name: "tag",
    run: (client, message, args, db) => {
        ...
        if (!tag_operations_1.options.includes(func))
        ...
        if (func === "create") {
            ...
            let name = args[0];
            if (!name)
                return message.channel.send("No tag name was given");
            if (!hasTag_1.hasTag(message.guild, name, db))
            ...
            let name = args[1];
            if (!name)
                return message.channel.send("No name was given");
            ...
            const embed = new discord_js_1.MessageEmbed()
                ...
                .addField("❯ Aliases", data.aliases.length ? data.aliases.join(", ") : "None")
                .addField("❯ Server", data.server.name)
                .addField("❯ Uses", data.uses)
                .addField("❯ Created", moment_1.default(data.createdAt).fromNow())
                .addField("❯ Last modified", moment_1.default(data.lastEditedAt).fromNow())
                .addField("❯ Last modified by", `${data.lastEditedBy.username} (${data.lastEditedBy.id})`)
            ...
            if (!tagName)
                return message.channel.send("No tag name was given");
            if (!alias)
                return message.channel.send("No alias name was given");
            if (!hasTag_1.hasTag(message.guild, tagName, db))
                return message.channel.send("That tag does not exist");
            let oldArray = db.get(`tags_${message.guild.id}.${tagName}.aliases`);
            oldArray.push(alias);
            db.set(`tags_${message.guild.id}.${tagName}.aliases`, oldArray);
            editLastModified_1.editLastModified(message.guild, tagName, message.author, db);
            ...
            db.set(`tags_${message.guild.id}.${tagName}.aliases`, oldArray);
            db.set(`tags_${message.guild.id}.${tagName}.lastEditedDone`, "Remove alias");
            editLastModified_1.editLastModified(message.guild, tagName, message.author, db);
            db.save();
            ...
            if (data.creator.id !== message.author.id &&
            ...
            db.set(`tags_${message.guild.id}.${tagName}.lastEditedDone`, "Changed response");
            db.save();
            ...
            let arr = [];
            if (!search)
                return message.channel.send("No search given");
